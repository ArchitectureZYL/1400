    ...
    <div class="relative w-full max-w-md">
      ...
      <div class="bg-white/80 backdrop-blur-lg rounded-2xl p-8 shadow-xl">
        ...
        <div class="mb-12 text-center">
          <h1 class="text-2xl font-semibold text-gray-800 mb-3">欢迎回来</h1>
          <p class="text-gray-500">请使用管理员提供的账号密码登录</p>
        </div>
        ...
        <div class="mt-4 text-center">
          <router-link to="/register"
                       class="text-[#00B4D8] hover:underline">没有账号，立即去注册</router-link>
        </div>
      </div>
    </div>
  </div>
...
const handleLogin = async () => {
  if (!username.value || !password.value) {
    const errorMessage = !username.value ? '请输入账号' : '请输入密码';
